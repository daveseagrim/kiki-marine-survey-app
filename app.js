  ...
  app.innerHTML = `
    ...
    <div class="content">
      ...
      <div class="form-group">
        ...
        <button class="btn-secondary" style="font-size:12px;margin-top:4px;padding:4px 10px;" onclick="updateValuationRationale(true)">🔄 Regenerate from sources</button>
      </div>
...
function updateValuationRationale(forceRegenerate) {
  const checkedSources = Array.from(document.querySelectorAll('.val-source:checked')).map(cb => cb.value);
  const rationaleEl = document.getElementById('valuationRationale');
  if (!rationaleEl) return;
  if (checkedSources.length === 0) {
    if (forceRegenerate) showToast('Check at least one valuation source first');
    return;
  }

  // Auto-generate only if: forced by button, or field is empty, or was previously auto-generated
  if (!forceRegenerate && rationaleEl.value && rationaleEl.dataset.autoGenerated !== 'true') return;
  ...
  if (forceRegenerate) showToast('Valuation rationale updated');
}
